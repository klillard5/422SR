...
module.exports = function(passport) {
    ...
    // DISPATCH LOGIN =============================================================
    // =========================================================================
    // we are using named strategies since we have one for dispatchLogin and one for signup
    // by default, if there was no name, it would just be called 'local'
    ...
    function(req, name, password, done) { // callback with name and password from our form
        ...
        // find a user whose name is the same as the forms name
        // we are checking to see if the user trying to dispatchLogin already exists
         // console.log(User.findOne({ 'local.name' :  name }));
        User.findOne({ 'local.name' :  name }, function(err, user) {
            ...
            if(true)							//**************THIS SHOULE BE  !user.local.dispatch == 1 ******
